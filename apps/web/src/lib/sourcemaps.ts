...
export async function storeSourceMap(input: {
  ...
  const projectRows = await sql<Array<{ id: string }>>`
    SELECT id FROM projects WHERE id = ${input.projectId} OR slug = ${input.projectId} LIMIT 1
  `;
  ...
  const countRows = await sql<Array<{ file_count: string }>>`
    ...
    WHERE project_id = ${project.id} AND release = ${input.release}
  ...
  await sql`
    ...
    VALUES (${project.id}, ${input.release}, ${input.fileName}, ${sql.json(JSON.parse(JSON.stringify(input.map)))})
